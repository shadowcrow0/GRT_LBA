...
* **ASYMMETRIC Person (Subject 36 - the "biased" brain):**
    * Often sees `|` as `\` on the left (left bias)
    * Often sees `|` as `/` on the right (right bias)
    * **Result:** 30.2% accuracy, extreme choice imbalance

### 🔍 What This Looks Like in Practice
When **Subject 36** sees this stimulus: `| |` (both vertical lines)
...
* **Left camera** (right hemisphere) processes right-side vision
* **Right camera** (left hemisphere) processes left-side vision

In asymmetric people:

* The **left camera** might be "tilted" - it sees verticals as diagonals.
* The **right camera** might be "tilted differently" - it sees verticals as the opposite diagonals.
* The two cameras don't match!
...
* When shown `|` `|`: Reports `\` `/` (wrong)
* When shown `|` `/`: Reports `\` `/` (accidentally right)
* **Pattern:** Almost always says `\` `/` regardless of reality.

### 💡 Why This Matters

* **For Science:**
    * Shows that bilateral brain processing isn't always symmetric.
    * Reveals individual differences in neural architecture.
    * Explains why some people struggle with spatial tasks.
...
* Your left brain and right brain see the world slightly differently, creating systematic biases in what you perceive on each side.
* In your experiment, this shows up as people consistently misidentifying line orientations in predictable, side-specific ways.
* Most people (10/18) have this asymmetry - it's actually normal to have somewhat unbalanced bilateral processing!
