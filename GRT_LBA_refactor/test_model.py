...
def lba_2dim_random(n_trials_per_condition, v_tensor, A, b, t0, s=1.0, rng=None):
    ...
    for cond in range(4):  # condition 0=VH, 1=HH, 2=HV, 3=VV
        ...
        for _ in range(n_trials_per_condition):
            ...
            # Filter: RT > 0 and valid (no upper limit)
            if rt > 0 and np.isfinite(rt):
...
def extract_trial_drift_rates(v_match_L, v_mismatch_L, v_match_R, v_mismatch_R, data):
    """
    Extract drift rates for each trial from posterior estimates and observed data
    (PS Model with Left/Right separation)

    Parameters:
    -----------
    v_match_L : float
        Posterior mean of v_match for Left dimension
    v_mismatch_L : float
        Posterior mean of v_mismatch for Left dimension
    v_match_R : float
        Posterior mean of v_match for Right dimension
    v_mismatch_R : float
        Posterior mean of v_mismatch for Right dimension
    data : array, shape (n_trials, 3)
    ...
    # Drift rate configuration for each condition (Left/Right separated)
    # v_tensor[cond, dimension, accumulator]
    ...
    drift_configs = {
        0: {  # VH: Left sees V, Right sees H
            'left_stim': 'V(/)',
            'right_stim': 'H(|)',
            'left_drift': [v_mismatch_L, v_match_L],  # [judge H, judge V]
            'right_drift': [v_match_R, v_mismatch_R]
        },
        1: {  # HH: Left sees H, Right sees H
            'left_stim': 'H(|)',
            'right_stim': 'H(|)',
            'left_drift': [v_match_L, v_mismatch_L],
            'right_drift': [v_match_R, v_mismatch_R]
        },
        2: {  # HV: Left sees H, Right sees V
            ...
            'right_stim': 'V(/)',
            'left_drift': [v_match_L, v_mismatch_L],
            'right_drift': [v_mismatch_R, v_match_R]
        },
        3: {  # VV: Left sees V, Right sees V
            'left_stim': 'V(/)',
            'right_stim': 'V(/)',
            'left_drift': [v_mismatch_L, v_match_L],
            'right_drift': [v_mismatch_R, v_match_R]
        }
    }
...
def analyze_and_save_results(trace, data, v_tensor, model_name="PS",
    ...
    if model_name == "PS":
        summary = az.summary(trace, var_names=["v_match_L", "v_mismatch_L", "v_match_R", "v_mismatch_R"])
        print("\nBasic Parameters (PS Model - Left/Right Separated):", flush=True)
    ...
    if model_name == "PS":
        print(f"   [PS Model] Posterior Estimates (Left/Right Separated):", flush=True)
        print(f"     Left:  v_match_L = {summary.loc['v_match_L','mean']:.3f}, v_mismatch_L = {summary.loc['v_mismatch_L','mean']:.3f}", flush=True)
        print(f"     Right: v_match_R = {summary.loc['v_match_R','mean']:.3f}, v_mismatch_R = {summary.loc['v_mismatch_R','mean']:.3f}", flush=True)
    ...
    if model_name == "PS":
        ...
        # Extract posterior mean (Left/Right separated)
        v_match_L_est = summary.loc['v_match_L', 'mean']
        v_mismatch_L_est = summary.loc['v_mismatch_L', 'mean']
        v_match_R_est = summary.loc['v_match_R', 'mean']
        v_mismatch_R_est = summary.loc['v_mismatch_R', 'mean']

        # Extract trial-level drift rates
        trial_df = extract_trial_drift_rates(v_match_L_est, v_mismatch_L_est,
                                              v_match_R_est, v_mismatch_R_est, data)
    ...
    if model_name == "PS":
        results = {
            ...
            'trial_df': trial_df,
            'v_match_L_est': v_match_L_est,
            'v_mismatch_L_est': v_mismatch_L_est,
            'v_match_R_est': v_match_R_est,
            'v_mismatch_R_est': v_mismatch_R_est,
...
def build_model_PS(observed_data):
    """
    PS Model with Left/Right separation (4 parameters)

    Parameters:
    - v_m_L, v_ms_L: Left dimension match/mismatch drift rates
    - v_m_R, v_ms_R: Right dimension match/mismatch drift rates

    Still maintains PS assumption: drift rates depend on stimulus type (H vs V),
    but allows left and right to differ.
    """
    with pm.Model() as model:
        A, b, s, t0 = 0.5, 1.5, 1.0, 0.25

        # ============================================================================
        # 4-parameter PS model: Left and Right separated
        # ============================================================================
        # Left dimension parameters
        vt_L = pm.HalfNormal("vt_L", sigma=2.0)
        ...
        init_vals = {"vt_L": 4.5, "vb_L": 0.7, "vt_R": 4.5, "vb_R": 0.7}
        trace_PS = pm.sample(draws=8000, tune=6+000, chains=24, step=pm.DEMetropolisZ(), random_seed=42, initvals=init_vals)
    return trace_PS
def build_model_noPS(observed_data):
    ...
    with pm.Model() as model:
        ...
        # ============================================================================
        # 重參數化 noPS：3 組參數 (VV, HH, Mixed)
        # ============================================================================
        # shape=3: [0]=VV, [1]=HH, [2]=Mixed (VH & HV 共用)
        vt = pm.HalfNormal("vt", sigma=2.0, shape=3)
        ...
        # VH 和 HV 共用 Mixed 參數
        v_m_VH = pm.Deterministic("v_match_VH", v_m[2])
        v_ms_VH = pm.Deterministic("v_mismatch_VH", v_ms[2])
        v_m_HV = pm.Deterministic("v_match_HV", v_m[2])
        v_ms_HV = pm.Deterministic("v_mismatch_HV", v_ms[2])

        # ============================================================================
        # Build v_tensor: 索引對應 [0]=VV, [1]=HH, [2]=Mixed
        # ============================================================================
        v_tensor = pt.zeros((4, 2, 2))
        # Cond 0: VH -> 用 Mixed [2]
        ...
        pm.Deterministic("log_likelihood", log_lik_vec)
        pm.Potential("obs", pt.sum(log_lik_vec))
        # init_vals: [0]=VV, [1]=HH, [2]=Mixed
        init_vals = {
            "vt": np.array([4.0, 3.0, 4.5]),  # VV, HH, Mixed
            "vb": np.array([0.7, 0.8, 0.7])   # VV, HH, Mixed
        }
        trace_noPS = pm.sample(
            draws=8000,    # 增加樣本數
            tune=6000,     # 給予更長的熱身期，讓 24 條鏈有時間匯合
            chains=24, 
...
if __name__ == "__main__":
    ...
    # PS Model: 左右分開 (4 parameters: v_m_L, v_ms_L, v_m_R, v_ms_R)
    # noPS Model: 3 組參數 ([0]=VV, [1]=HH, [2]=Mixed)

    # 為 PS 模型產生左右各一組參數
    v_m_L = rng.uniform(1.5, 3.5)
    v_ms_L = rng.uniform(0.5, 1.5)
    v_m_R = rng.uniform(1.5, 3.5)
    v_ms_R = rng.uniform(0.5, 1.5)

    # 為 noPS 模型產生 3 組參數
    v_m_random = rng.uniform(1.5, 3.5, size=3)     # [0]=VV, [1]=HH, [2]=Mixed
    v_ms_random = rng.uniform(0.5, 1.5, size=3)

    print("\n" + "="*70, flush=True)
    print("True Parameters for Data Generation", flush=True)
    print("="*70, flush=True)

    print("\nPS Model Parameters (Left/Right separated):", flush=True)
    print(f"  Left:  v_match_L={v_m_L:.2f}, v_mismatch_L={v_ms_L:.2f}", flush=True)
    print(f"  Right: v_match_R={v_m_R:.2f}, v_mismatch_R={v_ms_R:.2f}", flush=True)

    print("\nnoPS Model Parameters (3 groups: VV, HH, Mixed):", flush=True)
    print(f"  [0] VV:    v_match={v_m_random[0]:.2f}, v_mismatch={v_ms_random[0]:.2f}", flush=True)
    print(f"  [1] HH:    v_match={v_m_random[1]:.2f}, v_mismatch={v_ms_random[1]:.2f}", flush=True)
    print(f"  [2] Mixed: v_match={v_m_random[2]:.2f}, v_mismatch={v_ms_random[2]:.2f}", flush=True)

    # Construct v_tensor_PS with Left/Right separated structure
    # PS 模型：左右各有自己的 match/mismatch，但同一邊的所有 condition 共用參數
    # condition: 0=VH, 1=HH, 2=HV, 3=VV
    # accumulator: 0=H(|), 1=V(/)
    v_tensor_PS = np.array([
        [[v_ms_L, v_m_L], [v_m_R, v_ms_R]],  # VH: L sees V, R sees H
        [[v_m_L, v_ms_L], [v_m_R, v_ms_R]],  # HH: L sees H, R sees H
        [[v_m_L, v_ms_L], [v_ms_R, v_m_R]],  # HV: L sees H, R sees V
        [[v_ms_L, v_m_L], [v_ms_R, v_m_R]]   # VV: L sees V, R sees V
    ])
