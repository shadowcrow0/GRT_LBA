# -*- coding: utf-8 -*-
"""
Individual Subject GRT-LBA Model Analysis - COMPLETE FIXED VERSION
...
2. Improved MCMC sampling settings with quality control
3. Automatic retry mechanism for problematic subjects
"""
...
def check_sampling_quality(self, trace, subject_id):
    """Check MCMC sampling quality and provide diagnostics"""
    try:
        # Compute diagnostics
        ess = az.ess(trace)
        rhat = az.rhat(trace)
        ...
        min_ess = float(ess.min()) if hasattr(ess, 'min') else 100
        ...
        # Quality criteria
        quality_checks = {
            'min_ess_ok': min_ess >= 100,
            'rhat_ok': max_rhat <= 1.01,
            'divergences_ok': divergences == 0,
            'chains_ok': trace.posterior.dims.get('chain', 1) >= 2
        }
        
        acceptable = all(quality_checks.values())
        
        print(f"  Sampling Quality for Subject {subject_id}:")
        print(f"    Min ESS: {min_ess:.0f} ({'✓' if quality_checks['min_ess_ok'] else '✗'})")
        print(f"    Max R-hat: {max_rhat:.3f} ({'✓' if quality_checks['rhat_ok'] else '✗'})")
        print(f"    Divergences: {divergences} ({'✓' if quality_checks['divergences_ok'] else '✗'})")
        print(f"    Chains: {trace.posterior.dims.get('chain', 1)} ({'✓' if quality_checks['chains_ok'] else '✗'})")
        print(f"    Overall: {'✓ Acceptable' if acceptable else '✗ Needs improvement'}")
        ...
        print(f"  Warning: Could not compute sampling diagnostics: {e}")
        return {'acceptable': False, 'error': str(e)}

def resample_with_conservative_settings(self, model, draws, tune, subject_id):
    """Resample with more conservative MCMC settings"""
    print(f"  Attempting conservative resampling for subject {subject_id}...")
    
    with model:
        # Very conservative settings
        trace = pm.sample(
            draws=max(draws, 600),      # More draws
            tune=max(tune, 400),        # More tuning
            chains=4,                   # Always 4 chains for diagnostics
            progressbar=True,
            return_inferencedata=True,
            target_accept=0.99,         # Very high acceptance rate
            max_treedepth=15,           # Higher tree depth
            cores=1,
            random_seed=456 + subject_id,
...
def analyze_single_subject(self, subject_id, draws=400, tune=200, chains=2):
    """Improved single subject analysis with better MCMC settings"""
    print(f"\n{'='*50}")
    ...
    print(f"  Starting MCMC sampling with improved settings...")
    ...
    try:
        with model:
            ...
            try:
                trace = pm.sample(
                    draws=draws, 
                    tune=tune, 
                    chains=max(chains, 4),  # At least 4 chains
                    progressbar=True, 
                    return_inferencedata=True,
                    target_accept=0.95,     # Higher acceptance rate to reduce divergences
                    max_treedepth=12,       # Increase tree depth
                    cores=1, 
                    random_seed=123 + subject_id,
                    init='auto'             # Auto initialization
                )
                
                # Check sampling quality
                sampling_quality = self.check_sampling_quality(trace, subject_id)
                
                # If quality is poor, try more conservative settings
                if not sampling_quality['acceptable']:
                    print(f"  Initial sampling quality poor, trying conservative settings...")
                    trace = self.resample_with_conservative_settings(
                        model, draws, tune, subject_id)
                
            except Exception as e:
                print(f"  Standard sampling failed: {e}")
                print(f"  Trying conservative settings...")
                trace = self.resample_with_conservative_settings(
                    model, draws, tune, subject_id)
        ...
        # Final quality check
        final_quality = self.check_sampling_quality(trace, subject_id)
        ...
        if final_quality['acceptable']:
            ...
            print(f"  ✗ Sampling quality still poor for subject {subject_id}")
            return None
...
def retry_problematic_subjects(self, subject_ids, extended_draws=800, extended_tune=400):
    """Retry analysis for problematic subjects with extended settings"""
    print(f"\nRetrying {len(subject_ids)} subjects with extended MCMC settings...")
    
    retry_successful = 0
    
    for subject_id in subject_ids:
        print(f"\nRetrying subject {subject_id} with extended settings...")
        try:
            # Remove previous results if they exist
            if subject_id in self.individual_results:
                del self.individual_results[subject_id]
            if subject_id in self.individual_traces:
                del self.individual_traces[subject_id]
            
            # Retry with extended settings
            trace = self.analyze_single_subject(
                subject_id, 
                draws=extended_draws, 
                tune=extended_tune, 
                chains=4
            )
            
            if trace is not None:
                quality = self.check_sampling_quality(trace, subject_id)
                if quality['acceptable']:
                    retry_successful += 1
                    print(f"  ✓ Retry successful for subject {subject_id}")
                else:
                    print(f"  ⚠ Retry completed but still has quality issues for subject {subject_id}")
            else:
                print(f"  ✗ Retry failed for subject {subject_id}")
                
        except Exception as e:
            print(f"  ✗ Retry failed for subject {subject_id}: {e}")
    
    print(f"\nRetry summary: {retry_successful}/{len(subject_ids)} subjects improved")

def analyze_all_subjects(self, max_subjects=None, draws=400, tune=200):
    """Analyze all subjects individually with improved error handling"""
    print(f"\n{'='*70}")
    ...
    # Track subjects with issues for potential reanalysis
    subjects_with_issues = []
    
    for subject_id in subjects_to_analyze:
        try:
            trace = self.analyze_single_subject(subject_id, draws=draws, tune=tune, chains=4)
            
            if trace is not None:
                # Check if there were quality issues
                quality = self.check_sampling_quality(trace, subject_id)
                if quality['acceptable']:
                    successful_analyses += 1
                else:
                    quality_issues += 1
                    subjects_with_issues.append(subject_id)
                    print(f"  ⚠ Subject {subject_id} completed but with quality issues")
            else:
                failed_analyses += 1
                subjects_with_issues.append(subject_id)
    ...
    print(f"Success rate: {successful_analyses/(successful_analyses+failed_analyses+quality_issues)*100:.1f}%")
    
    if subjects_with_issues:
        print(f"\nSubjects with issues: {subjects_with_issues}")
        
        # Option to retry problematic subjects
        if len(subjects_with_issues) <= 5:  # Only for small numbers
            retry_choice = input(f"Retry {len(subjects_with_issues)} problematic subjects with extended settings? [y/n]: ")
            if retry_choice.lower() == 'y':
                print("Retrying with extended settings...")
                self.retry_problematic_subjects(subjects_with_issues, draws*2, tune*2)
    
    # Combine results if we have successful analyses
    if successful_analyses > 0:
        self.combine_results()

class IndividualSubjectGRTLBA:
    """Individual Subject GRT-LBA Analysis with FIXED likelihood function and improved MCMC"""
    ...
    def __init__(self, csv_file='GRT_LBA.csv'):
        ...
        # Add improved methods to the instance
        self.check_sampling_quality = types.MethodType(check_sampling_quality, self)
        self.resample_with_conservative_settings = types.MethodType(resample_with_conservative_settings, self)
        self.retry_problematic_subjects = types.MethodType(retry_problematic_subjects, self)
        self.analyze_single_subject = types.MethodType(analyze_single_subject, self)
        self.analyze_all_subjects = types.MethodType(analyze_all_subjects, self)
...
def save_combined_results(self):
    ...
    with open(combined_file, 'w') as f:
        json.dump(convert_numpy_types(self.combined_results), f, indent=2)
...
def run_individual_subject_analysis(max_subjects=None, draws=600, tune=400):
    """
    Main function to run individual subject GRT-LBA analysis with improved MCMC settings
    ...
    draws : int
        MCMC draws per subject (increased default)
    tune : int
        MCMC tuning steps per subject (increased default)
    ...
    print("Each subject analyzed separately with improved MCMC settings")
    print("="*70)
    ...
    # Run analysis with improved settings
    try:
